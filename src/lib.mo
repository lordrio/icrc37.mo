...
module {
  ...
  public class ICRC37(stored : ?State, canister : Principal, environment : Environment) {
    ...
    public func collection_approvals(owner : Account, prev : ?CollectionApproval, take : ?Nat) : Result.Result<[CollectionApproval], Text> {
      ...
      let sorted_accounts = Iter.sort<(?Nat, Account)>(
        Set.keys(approvals),
        func(a : (?Nat, Account), b : (?Nat, Account)) {
    ...
    public func update_ledger_info(request : [UpdateLedgerInfoRequest]) : [Bool] {
      ...
      for (thisItem in request.vals()) {
        switch (thisItem) {
          ...
          case (#MaxApprovalsPerTokenOrColletion(val)) {
            state.ledger_info.max_approvals_per_token_or_collection := val;
          };
          case (#MaxRevokeApprovals(val)) {
            state.ledger_info.max_revoke_approvals := val;
          };
          case (#MaxApprovals(val)) { state.ledger_info.max_approvals := val };
          case (#SettleToApprovals(val)) {
            state.ledger_info.settle_to_approvals := val;
          };
          case (#CollectionApprovalRequiresToken(val)) {
            state.ledger_info.collection_approval_requires_token := val;
          };
        };
        Vec.add(results, true);
      };
      ...
      label proc for (thisItem in revokeArgs.vals()) {
        ...
        let ?(memo) = testMemo(thisItem.memo) else {
          Vec.add(
            ...
            ? #Err(
              #GenericBatchError({
                message = "invalid memo. must be less than " # debug_show (environment.icrc7.get_ledger_info().max_memo_size) # " bits";
                error_code = 45;
              })
            ),
          );
    ...
    private func revoke_token_approval<system>(caller : Principal, revokeArg : RevokeTokenApprovalArg) : ?RevokeTokenApprovalResult {
      ...
      let ?(memo) = testMemo(revokeArg.memo) else return ? #Err(#GenericError({ error_code = 56; message = "illegal memo" }));
    ...
    private func transfer_token<system>(caller : Principal, transferFromArgs : TransferFromArg) : Result.Result<TransferFromResult, Text> {
      ...
      let ?transaction_result = environment.icrc7.finalize_token_transfer<system>(
        transferFromArgs.from.owner,
        {
          notification with
          from_subaccount = notification.from.subaccount
        } : ICRC7.TransferArg,
        trx,
        trxtop,
        notification.token_id,
      ) else return #ok(#Err(#GenericError({ error_code = 2345; message = "unreachable null transaction" })));
